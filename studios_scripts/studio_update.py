...
def update_inputs_via_autofill(channel, workspace_id, device, interface,
   ...
   source = 'generate'
   req = action_services.ActionExecConfigSetRequest(
      value=action_models.ActionExecConfig(
         key=action_models.ActionKey(
            id=wrappers.StringValue(value=action_id)
         ),
         exec_id=wrappers.StringValue(value=exec_id),
         dynamic_args=action_models.ActionArgValues(
         )
      )
   )
   execConfig = action_models.ActionExecConfig()
   ...
   stub = action_services.ActionExecConfigServiceStub(channel)
   stub.Set(req, timeout=RPC_TIMEOUT)
